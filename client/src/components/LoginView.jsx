...
import { ArrowRight, Car, ShieldCheck, Radio, Chrome, KeyRound, Eye, EyeOff } from 'lucide-react';
    ...
    try {
      ...
      if (firebaseEnabled) {
        ...
        try {
          ...
          if (mode === 'signup') throw firebaseError;
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md glass-panel border border-white/10 rounded-3xl p-7 shadow-2xl">
        ...
        <div className="relative mb-5">
          <input id="password" type={showPassword ? 'text' : 'password'} required value={password} onChange={(event) => setPassword(event.target.value)}
            className="w-full rounded-xl bg-black/30 border border-white/10 px-4 py-3 pr-11 text-sm outline-none focus:border-white/40" />
